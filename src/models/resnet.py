...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        out1 = self.fc1(torch.flatten(self.avgpool(x), 1))
        x = self.layer2(x)
        out2 = self.fc2(torch.flatten(self.avgpool(x), 1))
        x = self.layer3(x)
        out3 = self.fc3(torch.flatten(self.avgpool(x), 1))
        x = self.layer4(x)
        out4 = self.fc4(torch.flatten(self.avgpool(x), 1))
